...
def deep_map_from_raw(source: dict | list, destination_type: t.Type[T]) -> T:
    ...
    while True:
        ...
        if issubclass(common_destination_type, list) and isinstance(source, list):
            ...
            return [deep_map_from_raw(value, sub_destination_type) for value in source]
        ...
        if isinstance(source, dict):
            ...
            if is_dataclass(common_destination_type):
                ...
                for (
                    field_name,
                    field,
                ) in common_destination_type.__dataclass_fields__.items():
                    ...
                    if inspect.isclass(field.type):
                        ...
                        destination_type = field.type
                    result_dict[field_name] = deep_map_from_raw(
                        source[field_name], destination_type
                    )
